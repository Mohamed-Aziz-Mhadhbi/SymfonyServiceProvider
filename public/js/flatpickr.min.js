function Flatpickr(e, t) {
	...
	function M(e) {
		...
		if (c.appendChild(i), he.config.animate && void 0 !== e)
			for (; he.daysContainer.childNodes.length > 1;) he.daysContainer.removeChild(he.daysContainer.firstChild);
	...
	function T(e, t, n) {
		...
		if (!(a < 0 && he._hidePrevMonthArrow || a > 0 && he._hideNextMonthArrow)) {
			...
			if (a < 0)
				for (; r.nextSibling && /curr/.test(r.nextSibling.className);) he.monthNav.removeChild(r.nextSibling);
			else if (a > 0)
				for (; r.previousSibling && /curr/.test(r.previousSibling.className);) he.monthNav.removeChild(r.previousSibling);
	...
	function L(e) {
		...
		if (n || o || l) {
			...
			switch (e.key) {
				...
				case "ArrowRight":
					if (!c)
						if (e.preventDefault(), he.daysContainer) {
	...
	function U(e) {
		if (e.preventDefault(), e.stopPropagation(), e.target.classList.contains("flatpickr-day") && !e.target.classList.contains("disabled") && !e.target.classList.contains("notAllowed")) {
			...
			if (he.selectedDateElem = e.target, "single" === he.config.mode) he.selectedDates = [t];
			else if ("multiple" === he.config.mode) {
				var a = ae(t);
	...
	function z(e, t) {
		...
		else if (e instanceof Date || !isNaN(e)) he.selectedDates = [he.parseDate(e, t)];
		else if (e && e.substring) switch (he.config.mode) {
			case "single":
	...
	function te(e, t) {
		...
		if (void 0 !== n && n.length > 0)
			for (var a = 0; n[a] && a < n.length; a++) n[a](he.selectedDates, he.input.value, he, t);
	...
	var he = this;
	return he._ = {}, he._.afterDayAnim = b, he.changeMonth = T, he.changeYear = O, he.clear = _, he.close = S, he._createElement = de, he.destroy = Y, he.isEnabled = A, he.jumpToDate = g, he.open = H, he.redraw = K, he.set = $, he.setDate = G, he.toggle = ee,
		function () {
